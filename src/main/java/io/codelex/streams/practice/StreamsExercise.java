...
public class StreamsExercise {
    public static List<Integer> returnSquareRoot(List<Integer> numbers) {
        return numbers.stream()
                .map(number -> Double.valueOf(Math.sqrt(number)))
                .map(number -> number.intValue())
                .toList();
    }

    public static List<Integer> getAgeFromUsers(List<User> user) {
        return user.stream().map(age -> age.getAge()).toList();
    }
    ...
    public static boolean anyMatch(List<User> users, int age) {
        return users.stream().anyMatch(userAge -> userAge.getAge() == age);
    }

    public static boolean noneMatch(List<User> users, int age) {
        return users.stream().noneMatch(userAge -> userAge.getAge() == age);
    }
    ...
    public static List<Integer> generate10RandomNumbers() {
        return new Random().ints(10, 0, Integer.MAX_VALUE)
                .boxed()
                .toList();
    }
